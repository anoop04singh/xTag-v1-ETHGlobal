...
const NftAttribute = ({ trait_type, value }) => (
  <div className="flex justify-between items-center text-xs p-2 bg-zinc-50 dark:bg-zinc-800/50 rounded-md">
    <span className="text-zinc-500 dark:text-zinc-400">{trait_type || 'Property'}</span>
    <span className="font-medium text-right">{String(value) || 'N/A'}</span>
  </div>
);
...
const NftCard = ({ nft }) => {
  ...
  if (typeof attributes === 'string' && attributes.trim().startsWith('[')) {
    try {
      ...
      console.error("Failed to parse NFT attributes string:", e);
      attributes = [];
    }
  }
